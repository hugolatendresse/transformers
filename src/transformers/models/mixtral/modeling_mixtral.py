...
class MixtralSparseMoeBlock(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        router_logits = self.gate(hidden_states) # (batch x seq_len) x n_experts

        routing_weights = F.softmax(router_logits, dim=1, dtype=torch.float) # (batch x seq_len) x n_experts
        routing_weights, selected_experts = torch.topk(routing_weights, self.top_k, dim=-1)  # (batch x seq_len) x top_k weights, (batch x seq_len) x top_k indices
        routing_weights /= routing_weights.sum(dim=-1, keepdim=True) # (batch x seq_len) x top_k normalized weights
        # we cast back to the input dtype
        routing_weights = routing_weights.to(hidden_states.dtype)  # (batch x seq_len) x top_k normalized weights
        ...
        expert_mask = torch.nn.functional.one_hot(selected_experts, num_classes=self.num_experts).permute(2, 1, 0) #  n_experts x top_k x (batch x seq_len)

        # Loop over all available experts in the model and perform the computation on each expert
        for expert_idx in range(self.num_experts):
            expert_layer = self.experts[expert_idx]
            idx, top_x = torch.where(expert_mask[expert_idx])  # MixtralBlockSparseTop2MLP (just a 2-layer MLP)
            ...
            current_state = hidden_states[None, top_x].reshape(-1, hidden_dim)# (number of tokens using this expert) x hidden_dim
            current_hidden_states = expert_layer(current_state) * routing_weights[top_x, idx, None] # (number of tokens using this expert) x hidden_dim

            # However `index_add_` only support torch tensors for indexing so we'll use
            # the `top_x` tensor here.
            final_hidden_states.index_add_(0, top_x, current_hidden_states.to(hidden_states.dtype))  # (batch_size x sequence_length) x hidden_dim
        final_hidden_states = final_hidden_states.reshape(batch_size, sequence_length, hidden_dim)
        return final_hidden_states, router_logits # batch_size x sequence_length x hidden_dim,   (batch x seq_len) x n_experts
